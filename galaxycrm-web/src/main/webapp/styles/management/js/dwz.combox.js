...
(function($){
	...
	var _onchange2 = function (event){
		if (!event.data.actionUrl) 
			return false;
		$.ajax({
			type:'POST', dataType:"json", url:event.data.actionUrl.replace("{value}", encodeURIComponent(event.data.$this.attr("value"))), cache: false,
			data:{},
			success: function(json){
				if (!json) return;
				dialogAjaxDone(json);
		...
		combox:function(){
			...
			return this.each(function(i){
				...
				if (ref && refUrl) {
					...
					$this.unbind("change", _onchange2).bind("change", {actionUrl:actionUrl, $this:$this}, _onchange2);
				}
