 ...
 * @date 2015年10月23日
 */
@Service("ActivityAccessTotalCountAO")
@Slf4j
public class ActivityAccessTotalCountAO extends BaseAO {
	@Resource
	private ActivityAccessTotalCountManager activityAccessTotalCountManager;
	 ...
	 * @return 对象ID
	 */
	public Result<Long> save(ActivityAccessTotalCountDO obj) {
		Result<Long> result = new Result<Long>(false);
		try {
			if (obj == null) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_NULL);
			}
			Long id = activityAccessTotalCountManager.insert(obj);
			result.setValue(id);
	 ...
	 * @return 更新成功的记录数
	 */
	public Result<Integer> update(ActivityAccessTotalCountDO obj) {
		Result<Integer> result = new Result<Integer>(false);
		try {
			if (obj == null) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_NULL);
			}
			int num = activityAccessTotalCountManager.update(obj);
			if (num > 0) {
				result.setValue(num);
				result.setSuccess(true);
			} else {
	...
	public Result<Integer> deleteById(Long id) {
		...
		try {
			...
			int num = activityAccessTotalCountManager.deleteById(id);
			if (num > 0) {
				result.setValue(num);
				result.setSuccess(true);
			} else {
	...
	public Result<Integer> deleteByType(int type) {
		Result<Integer> result = new Result<Integer>(false);
		try {
			if (!NumUtil.isGreaterZero(type)) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_INVALID);
			}
			int num = activityAccessTotalCountManager.deleteByType(type);
			if (num > 0) {
				result.setValue(num);
				result.setSuccess(true);
			} else {
			...
			log.error("deleteByType error," + type, e);
			if (e instanceof MyException) {
				setErrorMessage(result, ((MyException) e).getResultCode());
			} else {
				setErrorMessage(result, ResultCode.SYSTEM_ERROR);
	...
	public Result<Integer> deleteByDate(String date) {
		Result<Integer> result = new Result<Integer>(false);
		try {
			int num = activityAccessTotalCountManager.deleteByDate(date);
			if (num > 0) {
				result.setValue(num);
				result.setSuccess(true);
			} else {
			...
			log.error("deleteByDate error," + date, e);
			if (e instanceof MyException) {
				setErrorMessage(result, ((MyException) e).getResultCode());
			} else {
				setErrorMessage(result, ResultCode.SYSTEM_ERROR);
	...
	public Result<Integer> deleteByQuery(ActivityAccessTotalCountQuery q) {
		Result<Integer> result = new Result<Integer>(false);
		try {
			int num = activityAccessTotalCountManager.deleteByQuery(q);
			if (num > 0) {
				result.setValue(num);
				result.setSuccess(true);
			} else {
	...
	public Result<List<ActivityAccessTotalCountVO>> statistics(ActivityAccessTotalCountQuery query) {
		Result<List<ActivityAccessTotalCountVO>> result = new Result<List<ActivityAccessTotalCountVO>>(false);
		try {
			if (query == null) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_INVALID);
			}
			List<ActivityAccessTotalCountVO> list = activityAccessTotalCountManager.statistics(query);
