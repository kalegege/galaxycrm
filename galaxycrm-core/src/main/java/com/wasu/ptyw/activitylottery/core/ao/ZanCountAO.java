...
@Service("zanCountAO")
@Slf4j
public class ZanCountAO extends BaseAO {
	@Resource
	private ZanCountManager zanCountManager ;
	 ...
	 * @return 更新成功的记录数
	 */
	public Result<Integer> update(ZanCountDO obj) {
		Result<Integer> result = new Result<Integer>(false);
		try {
			if (obj == null) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_NULL);
			}
			int num = zanCountManager.update(obj);
			if (num > 0) {
				result.setValue(num);
				result.setSuccess(true);
			} else {
	...
	public Result<Integer> deleteById(Long id) {
		...
		try {
			...
			int num = zanCountManager.deleteById(id);
			if (num > 0) {
				result.setValue(num);
				result.setSuccess(true);
			} else {
	 ...
	 * @return VoteCountDO
	 */
	public Result<ZanCountDO> getById(Long id) {
		Result<ZanCountDO> result = new Result<ZanCountDO>(false);
		try {
			if (!NumUtil.isGreaterZero(id)) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_INVALID);
			}
			ZanCountDO obj = zanCountManager.getById(id);
			if (obj != null) {
				result.setValue(obj);
				result.setSuccess(true);
			} else {
	 ...
	 */
	public Result<List<ZanCountDO>> getByIds(List<Long> ids) {
		Result<List<ZanCountDO>> result = new Result<List<ZanCountDO>>(false);
		try {
			if (ids == null || ids.isEmpty()) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_INVALID);
			}
			List<ZanCountDO> list = zanCountManager.getByIds(ids);
	 ...
	 * @return List<ActivityIntroductionDO>
	 */
	public Result<List<ZanCountDO>> query(ZanCountQuery query) {
		Result<List<ZanCountDO>> result = new Result<List<ZanCountDO>>(false);
		try {
			if (query == null) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_INVALID);
			}
			List<ZanCountDO> list = zanCountManager.query(query);
			result.setValue(list);
			result.setSuccess(true);
		} catch (Exception e) {
	 ...
	 * @return List<ActivityIntroductionDO>
	 */
	public Result<List<ZanCountDO>> queryAll(ZanCountQuery query) {
		Result<List<ZanCountDO>> result = new Result<List<ZanCountDO>>(false);
		try {
			if (query == null) {
				return setErrorMessage(result, ResultCode.PARAM_INPUT_INVALID);
			}
			List<ZanCountDO> list = zanCountManager.queryAll(query);
			result.setValue(list);
			result.setSuccess(true);
		} catch (Exception e) {
			log.error("query error," + query, e);
	...
	public Result<Integer> updateStatusByIds(List<Long> ids, int status) {
		...
		try {
			...
			int num = zanCountManager.updateStatusByIds(ids, status);
			result.setValue(num);
			result.setSuccess(true);
		} catch (Exception e) {
